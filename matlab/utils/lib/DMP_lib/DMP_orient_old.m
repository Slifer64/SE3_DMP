...
classdef DMP_orient_old < handle % : public DMP_
    ...
    methods
        ...
        function this = DMP_orient_old(N_kernels, a_z, b_z, can_clock_ptr, shape_attr_gat_ptr, dmp_type)
            
            for i=1:3
                this.dmp{i} = DMP(DMP.TYPE,dmp_type ,DMP.NKernels,N_kernels(i), DMP.Az,a_z(i), DMP.Bz,b_z(i), DMP.CanClock,can_clock_ptr, DMP.ShapeAttrGat,shape_attr_gat_ptr);
            end

            this.can_clock_ptr = can_clock_ptr;
            this.shape_attr_gating_ptr = shape_attr_gat_ptr;
